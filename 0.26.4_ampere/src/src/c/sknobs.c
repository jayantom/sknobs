...
// $Author: jscheid $
// $Date: 2017/12/05 $
// $Id: //depot/users/jscheid/src/sknobs/0.26/src/c/sknobs.c#1 $
//
////////////////////////////////////////////////////////////////////////////
...
int sknobs_add(char *pattern, char *value, char *comment) {
  ...
  if (value) {
    char new_value_buffer[SKNOBS_MAX_LENGTH];
    char *new_value = new_value_buffer;
    if (sknobs_repl_env_var(value, new_value)) //evaluate any env var's in value
...
////////////////////////////////////////////////////////////////////////////
// repl_env_var
// Evaluate an environment variable in a sknob value and update the value of
// the sknob variable accordingly.
// Returns the updated sknob value
//
// sknob value must be in the following format in order to indicate
// that the environment variable in the sknob value should be evaluated and
// and replaced by the value of the environment variable: env(ENV_VAR)
// For example, if the MODULEHOME env variable was set to:
//  /tools/modulefiles/modules/3.2.10, then this sknob:
//  +sknob.my_path=env(MODULEHOME)/Modules/versions
//  would be changed to:
//  +sknob.my_path=/tools/modulefiles/modules/3.2.10/Modules/versions
int sknobs_repl_env_var(char *orig_value, char *new_value) {
  char c; // temp charcter variable
  char *index, *env_var_value_index;
  char *env_var_value;
  char var_name[1024];
  char *var_name_index = var_name;
  enum {NORMAL, ENV_VAR} state;
  // skip locking character
  while ((orig_value[0] == '='))
    ++orig_value;
  // evaluate any "env(*)" expressions and replace with env var value
  index = orig_value;
  state = NORMAL;
  while ((c = *index)) {
    switch (state) {
    case NORMAL:
      if (!strncmp(index, "env(", 4)){ //strncmp returns 0 if match found
	state = ENV_VAR;
        index+=3; // advance past "env("
      }
      else
        *new_value++ = c;
      break;
    case ENV_VAR:
      if (c == ')') { // end character detected
	*var_name_index = '\0';
	env_var_value = getenv(var_name);
        // "reset" var_name in case there are multiple env vars to evaluated in
        // the same sknob
        var_name_index = var_name;
        if (!env_var_value) {
           printf("error: sknobs: no value for env var %s\n", var_name);
           return 1;
        }
	// copy env var value to buffer for new sknob value
	env_var_value_index = env_var_value;
        int i = 0;
	while ((c = *env_var_value_index)) {
	  *new_value++ = c;
	  env_var_value_index++;
          i++;
	}
  ...
  if (state != NORMAL) {
    printf("error: sknobs: incorrect env var function format, missing )?\n");
    return 1;
  }
  *new_value = '\0'; // terminate the string for the new value
  return 0; // success
}
...
int sknobs_add_string(char *s, char *comment) {
  ...
  //NULL tells string token function to continue withe the previously 
  //passed string
  value = strtok(NULL, "\0");
  if (!value)
    value = "1";
