...
// $Author: jscheid $
// $Date: 2017/12/05 $
// $Id: //depot/users/jscheid/src/sknobs/0.26/src/python/sknobs_ext.c#1 $
//
////////////////////////////////////////////////////////////////////////////
...
init(PyObject *self, PyObject *args) {
  ...
  for (i=0; i<argc; ++i) {
    ...
    argv[i] = PyUnicode_AsUTF8(s);
  }
  argv[i] = 0;
  return PyLong_FromLong(sknobs_init(argc, argv)); 
}

////////////////////////////////////////////////////////////////////////////
static PyObject *
exists(PyObject *self, PyObject *args) {
  char *name = PyUnicode_AsUTF8(args);
  return PyLong_FromLong(sknobs_exists(name)); 
}
...
get_string(PyObject *self, PyObject *args) {
  ...
  if(!PyArg_ParseTuple(args,(char *)"ss:get_string",&name, &defaultValue)) 
    return NULL;
  return PyUnicode_FromString(sknobs_get_string(name, defaultValue));
}
...
find_file(PyObject *self, PyObject *args) {
  char *filename = PyUnicode_AsUTF8(args);
  return PyUnicode_FromString(strdup(sknobs_find_file(filename)));
}
...
get_filename(PyObject *self, PyObject *args) {
  ...
  if(!PyArg_ParseTuple(args,(char *)"ss:get_string",&name, &defaultValue)) 
    return NULL;
  return PyUnicode_FromString(sknobs_get_filename(name, defaultValue));
}
...
////////////////////////////////////////////////////////////////////////////
static struct PyModuleDef sknobs_module = {
    PyModuleDef_HEAD_INIT,
    "skonbs",            /* m_name */
    "sknobs extension module",  /* m_doc */
    -1,                  /* m_size */
...
PyMODINIT_FUNC
PyInit_sknobs(void) {
  return PyModule_Create(&sknobs_module);
}
